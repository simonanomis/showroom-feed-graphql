...
import { Showroom, ShowroomModel } from '../../entities/Showroom';
...
import { Board, BoardModel } from '../../entities/Board';
  ...
  export class ShowroomResolver {
    @Query((_returns) => Showroom, { nullable: false })
    async returnSingleShowroom(@Arg('id') id: string) {
      return await ShowroomModel.findById({ _id: id });
    }
    ...
    @Mutation(() => Showroom)
    async createShowroom(@Arg('data') { boards }: ShowroomInput): Promise<Showroom> {
      const board = (
    ...
    async deleteShowroom(@Arg('id') id: string) {
      await ShowroomModel.deleteOne({ id });
      return true;
    }
